...
import torch
import time
from MDTEformer_exp.exp_MDTEformer import Exp_MDTEformer
from utils.tools import string_split

parser = argparse.ArgumentParser(description='MDTEformer')

parser.add_argument('--data', type=str, default='Australia', help='data')
parser.add_argument('--data_path', type=str, default='Australia.csv', help='data file')
...
parser.add_argument('--in_len', type=int, default=24, help='input MTS length (T)')
...
parser.add_argument('--batch_size_max', type=int, default=128, help='max batch size of train input data')
parser.add_argument('--batch_size_min', type=int, default=32, help='min batch size of train input data')
...
data_parser = {
    'Australia': {'data': 'Australia.csv', 'data_dim': 4, 'split': [0.7, 0.1, 0.2], 'day': 48},
    'GEFC2012': {'data': 'GEFC2012.csv', 'data_dim': 2, 'split': [0.7, 0.1, 0.2], 'day': 24},
    'UK': {'data': 'UK.csv', 'data_dim': 5, 'split': [0.7, 0.1, 0.2], 'day': 48},
    'European': {'data': 'European.csv', 'data_dim': 15, 'split': [0.7, 0.1, 0.2], 'day': 24},
}
...
if args.data in data_parser.keys():
    ...
    args.in_len = data_info['day']
    args.out_len = data_info['day']
else:
...
for ii in range(args.itr):
    ...
    setting = 'MDTEformer_{}_il{}_ol{}_sl{}_win{}_fa{}_dm{}_nh{}_el{}_itr{}'.format(args.data,
